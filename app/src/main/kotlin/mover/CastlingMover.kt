...
class CastlingMover(override val validators: List<Rule>) : Mover {
    ...
    override fun move(board: Board, movement: Movement): Board {
        ...
        newBoard.printBoard()
        val whiteShortCastling = movement.to.x > movement.from.x && movement.to.y == board.getNumberOfRows()
        ...
        return if (blackShortCastling) {
            val rookBoard = newBoard.changeWithPiece(
                board.getSquare(Position(movement.to.x - 1, movement.to.y)).getPiece(),
            ...
            val rookBoard = newBoard.changeWithPiece(
                board.getSquare(Position(movement.to.x + 2, movement.to.y)).getPiece(),
                Position(movement.to.x - 1, movement.to.y)
            )
